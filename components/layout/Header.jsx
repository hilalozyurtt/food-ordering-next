  ...
  return (
    <div className='h-[5.5rem] bg-secondary'>
      <div className='container mx-auto text-white flex justify-between items-center h-full'>
        <Logo/>
        <nav className={`sm:static absolute top-0 left-0 grid place-content-center 
         sm:w-auto sm:h-auto w-full h-full sm:text-white text-black bg-white sm:bg-transparent
         ${isMenuModal !== true && "sm:flex hidden"}`}>
        ...
        <div className='flex gap-x-4 items-center'>
            ...
            <button href='#' onClick={() => setIsSearchModal(true)}><FaSearch className='hover:text-primary transition-all'/></button>
            <a href='#' className="md:inline-block hidden"><button className='btn-primary'> Order Online </button></a> 
            <button className='sm:hidden inline-block' onClick={() => setIsMenuModal(true)}><GiHamburgerMenu className='hover:text-primary transition-all text-xl'/></button>
        </div>
      </div>
